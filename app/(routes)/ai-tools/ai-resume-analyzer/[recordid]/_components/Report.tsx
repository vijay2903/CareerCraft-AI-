        ...
        <div className='p-5'>
            ...
            <div className="bg-gradient-to-r from-[#BE575F] via-[#A338E3] to-[#AC76D6] rounded-lg shadow-md p-6 mb-6 border border-blue-200 transform hover:scale-[1.01] transition-transform duration-300 ease-in-out">
                ...
                <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center">
                        ...
                        <span className="text-yellow-300 text-lg font-bold">{aiReport?.overall_feedback}</span>
                    </div>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2.5 mb-4">
                    <div className="bg-white h-2.5 rounded-full" ></div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                {['contact_info', 'experience', 'education', 'skills'].map((sectionKey) => {
                    ...
                    return (
                        <div key={sectionKey} className={`bg-white rounded-lg shadow-md p-5 border relative overflow-hidden group ${borderColor}`}>
                            ...
                            <div className={`absolute inset-x-0 bottom-0 h-1 bg-${color}-500 opacity-0 group-hover:opacity-100 transition-opacity duration-300`}></div>
                        </div>
                    );
                })}
            </div>
                ...
                <ol className="list-none space-y-4">
                    {aiReport?.tips_for_improvement.map((item: any, index: number) => (
                        <li key={index} className="flex items-start gap-4">
                            <span className="w-6 h-6 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center font-bold"><i className="fas fa-check"></i></span>
